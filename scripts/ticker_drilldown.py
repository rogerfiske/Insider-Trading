...
def generate_ticker_report(ticker: str, output_path: Path | None = None) -> str:
    ...
    lines = [
        ...
        "## Executive Summary",
        "",
        f"This report exercises the seven-agent insider-trading framework for ticker `{ticker}`. Due to current connector limitations, most agents cannot filter to ticker-specific data and instead analyze all recent data or report not applicable.",
        "",
        ...
        f"- Cannot determine if any of the {len(form4_result.evidence) if form4_result.ok else 0} filings are for MAIA",
        "",
        "**Limitation**: Ticker-to-CIK resolution not implemented",
        "",
        "**What Would Be Needed**:",
        "1. Implement ticker-to-CIK lookup (e.g., via SEC company tickers JSON)",
        f"2. Filter Form 4 results to CIK for {ticker}",
        ...
        f"- Filings retrieved: {len(form4_result.evidence) if form4_result.ok else 0} (last 24 hours)",
        f"- MAIA-specific filings: Cannot determine (ticker filtering not implemented)",
        "",
        "**SEC 13F**:",
        ...
        f"- MAIA holdings: Cannot determine (ticker/CUSIP filtering not implemented)",
        "",
        "---",
        "",
        "## Limitations",
        ...
        "## Conclusion",
        "",
        f"This diagnostic report demonstrates the current insider-trading framework's structure and agent roles for ticker `{ticker}`. While the framework is operational, current connectors do not support ticker-specific filtering.",
        "",
        "**Next Steps**:",
