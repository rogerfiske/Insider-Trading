# MAIA Biotechnology, Inc. — Monitoring Baseline Status

**Ticker:** MAIA
**CIK:** 0001878313
**Generated:** 2026-06-10
**Checkpoint:** CP23E
**Based on:** CP23D MAIA Full Synthesis Packet

---

## Current Baseline Values (as of 2026-06-10)

### Insider Activity

| Metric | Value |
|--------|-------|
| Open-Market Purchases | 134 |
| Open-Market Sales | 0 |
| Purchase Value | $4,921,437.58 |
| Distinct Buyers | 10 |
| Latest Purchase Date | 2026-06-01 (8 days ago) |
| Insider Score | 100 / 100 |
| Rating | Very Strong Insider Buying Evidence |

**Status:** ✅ Perfect insider buying pattern - zero sales, broad participation, recent activity

### Capital Structure

| Metric | Value |
|--------|-------|
| Common Shares Outstanding | 60,671,491 (as of March 31, 2026) |
| Stock Options | 13,097,991 |
| Warrants | 13,086,220 |
| Approximate Fully Diluted | ~86.86M shares |
| Dilution Overhang | ~43% if all exercised |
| March 2026 Offering Price | $1.50 per share |

**Status:** ⚠️ Moderate dilution overhang (~43%) from options/warrants

### Official Q1 2026 Financials

| Metric | Value |
|--------|-------|
| Period Ended | March 31, 2026 |
| Cash and Cash Equivalents | $34,413,110 |
| Working Capital | $28,992,690 |
| Q1 2026 Operating Cash Burn | $5,311,328 quarterly |
| Base Runway | 19.4 months from March 31, 2026 |
| Estimated Depletion Date | January 2028 (base case) |

**Status:** ✅ Strong cash position post-March 2026 raise, adequate near-term runway

### Clinical Status

| Program | Status |
|---------|--------|
| THIO-104 | Phase 3 pivotal trial, Advanced NSCLC (2L+), FDA Fast Track |
| THIO-101 | Phase 2 expansion, indication not disclosed |
| THIO-104 Data Timing | **NOT DISCLOSED** ⚠️ (critical unknown) |

**Status:** ⚠️ Fast Track granted (positive), but data timing not disclosed (critical unknown)

### Institutional Visibility

| Category | Status |
|----------|--------|
| Form 13D/13G Filings | 0 |
| Form 144 Filings | 0 |
| 13F Integration | Implemented (CP23F) - Parser hardened (CP23F-Fix) |
| 13F Parser Success Rate | 60% (3 of 5 managers parsed successfully) |
| 13F Holdings Parsed | 21,128 total positions from 3 managers |
| MAIA Matches Found | 0 in Q1 2026 sample |
| 13F Parsing Failures | 2 managers (Berkshire Hathaway, Renaissance Technologies - non-standard format) |

**Status:** 13F InfoTable parser hardened in CP23F-Fix. 3 of 5 reviewed manager InfoTables parsed successfully. No reliable MAIA matches found among successfully parsed managers. Berkshire Hathaway and Renaissance Technologies remain unresolved due to non-standard/HTML-wrapper-only InfoTable access. Institutional visibility remains partial.

---

## Current Monitoring Posture

### Overall Research Posture

**Strong insider-evidence / high clinical-timing uncertainty profile**

**Component Scores (from CP23D):**
- Insider Evidence: 100 / 100 (Very Strong)
- Dilution/Capital Risk: 65 / 100 (Moderate)
- Cash Runway: 85 / 100 (Strong)
- Clinical Progress: 60 / 100 (Moderate - limited disclosure)
- Data Quality/Confidence: 90 / 100 (High - official SEC sources only)

### Monitoring Status by Category

1. **Insider Activity:** ✅ ACTIVE - Weekly Form 4 monitoring for continued buying or first sale signal
2. **Form 144:** ✅ ACTIVE - As-filed monitoring for first insider sale notice
3. **13D/G Ownership:** ✅ ACTIVE - As-filed monitoring for first 5%+ holder
4. **13F Institutional:** ⚠️ PARTIAL - Parser hardened (60% success rate), manual checks continue
5. **Clinical/Regulatory:** ✅ ACTIVE - As-issued monitoring for THIO-104 data timing disclosure
6. **Cash Runway:** ✅ ACTIVE - Quarterly monitoring for burn rate and runway changes
7. **Market Confirmation:** ⚠️ LIMITED - Manual price checks only (automation pending)
8. **Alert Separation:** ✅ VERIFIED - Report-only monitoring (no alerts generated)

---

## Top 5 Items to Watch Next

### 1. THIO-104 Phase 3 Data Timing Disclosure

**Why It Matters:** Critical unknown that determines financing risk and investment timeline

**What to Watch:**
- MAIA IR press releases
- SEC Form 8-K material event filings
- Conference presentations (ASCO, ASH, ESMO)
- ClinicalTrials.gov status changes

**PM Review Trigger:** Data timing disclosed → **REQUIRES IMMEDIATE PM REVIEW**

**Status:** ⚠️ NOT DISCLOSED (highest priority watch item)

---

### 2. First Open-Market Insider Sale After Zero-Sales Baseline

**Why It Matters:** Would break perfect 100/100 insider buying pattern

**What to Watch:**
- Weekly Form 4 filings (CIK 0001878313)
- Daily Form 4 RSS feed monitoring (if automated)

**PM Review Trigger:** First sale → **REQUIRES IMMEDIATE PM REVIEW**

**Status:** ✅ 0 sales to date (134 purchases, 0 sales baseline)

---

### 3. Cash Runway vs. THIO-104 Data Timing

**Why It Matters:** Determines if additional dilutive raise needed before catalyst

**What to Watch:**
- Quarterly 10-Q cash and burn rate updates
- Any new financing filings (S-3, 424B5, private placement)
- Going-concern language in 10-Q/10-K

**PM Review Triggers:**
- Runway drops below 12 months → **REQUIRES IMMEDIATE PM REVIEW**
- New financing filing → **REQUIRES IMMEDIATE PM REVIEW**
- Going-concern warning → **REQUIRES IMMEDIATE PM REVIEW**

**Status:** ✅ 19.4 months base runway (adequate near-term), but unknown if sufficient to reach THIO-104 data

---

### 4. First Form 144 Filing After Zero-Filings Baseline

**Why It Matters:** Would signal insider sale intent (Form 144 required before restricted stock sale)

**What to Watch:**
- Weekly Form 144 filings (CIK 0001878313)
- As-filed Form 144 RSS feed monitoring (if automated)

**PM Review Trigger:** First Form 144 filing → **REQUIRES IMMEDIATE PM REVIEW**

**Status:** ✅ 0 filings to date (zero-filings baseline)

---

### 5. First 13D/13G Filing After Zero-Filings Baseline

**Why It Matters:** Would reveal new 5%+ institutional holder (potential positive signal if reputable investor)

**What to Watch:**
- Weekly Form 13D/13G filings (CIK 0001878313)
- As-filed 13D/13G RSS feed monitoring (if automated)

**PM Review Trigger:** First 13D/13G filing → **REQUIRES IMMEDIATE PM REVIEW**

**Status:** ⚠️ 0 filings to date, but beneficial ownership blockers (4.99%/9.99%) prevent most investors from reaching 5% threshold

**Note:** Absence of 13D/13G filings may not reflect true institutional accumulation due to blockers.

---

## What Would Trigger PM Review

### Critical Triggers (Immediate PM Review Required)

1. ⚠️ **FIRST OPEN-MARKET INSIDER SALE** after zero-sales baseline
2. ⚠️ **FIRST FORM 144 FILING** after zero-filings baseline
3. ⚠️ **THIO-104 DATA TIMING DISCLOSED** (resolves critical unknown)
4. ⚠️ **THIO-104 TOPLINE DATA RELEASED**
5. ⚠️ **REGULATORY SUBMISSION ANNOUNCED** (BLA/NDA filing)
6. ⚠️ **TRIAL HALTED** or **FUTILITY ANALYSIS DISCLOSED**
7. ⚠️ **CASH RUNWAY DROPS BELOW 12 MONTHS**
8. ⚠️ **NEW FINANCING FILING** (S-3, 424B5, private placement)
9. ⚠️ **GOING-CONCERN WARNING APPEARS** in 10-Q/10-K

### High-Priority Triggers (PM Review Within 48 Hours)

10. ⚠️ **FIRST 13D/13G FILING** (new 5%+ holder)
11. ⚠️ **NEW 13F POSITION** by reputable biotech investor (if infrastructure integrated)
12. ⚠️ **LARGE-SCALE INSIDER SALE** (>1% of shares outstanding)
13. ⚠️ **BURN RATE INCREASES >50%** vs. Q1 2026 baseline ($5.31M quarterly)
14. ⚠️ **SUSTAINED PRICE DECLINE >50%** from $1.50 offering price

### Medium-Priority Triggers (PM Review at Next Weekly Check)

15. 🔔 New insider purchases (positive development)
16. 🔔 Clinical enrollment progress disclosed
17. 🔔 FDA Fast Track updates or regulatory pathway clarifications
18. 🔔 Price holding above $1.50 offering price with sustained volume

---

## What Is Not Yet Automated

### High-Priority Engineering Follow-Ups

1. **13F InfoTable XML Matching Integration**
   - **Status:** ✅ COMPLETED (CP23F) - No MAIA matches found in current sample
   - **Result:** Implemented and validated; reviewed 5 large managers (Q1 2026)
   - **Next Steps:** Expand to more managers, obtain MAIA CUSIP for higher confidence
   - **Recommendation:** Continue manual quarterly 13F checks; automate quarterly script runs

2. **Market Price/Volume Tracking**
   - **Status:** Not implemented
   - **Impact:** Cannot assess price confirmation vs. insider buying automatically
   - **Workaround:** Manual check of Yahoo Finance or similar public source
   - **Recommendation:** Integrate market data feed for automated daily price/volume tracking

### Medium-Priority Engineering Follow-Ups

3. **Form 4 Daily Scraping and Parsing**
   - **Status:** Manual weekly checks
   - **Impact:** Delayed detection of insider transactions (weekly vs. daily)
   - **Recommendation:** Automate daily Form 4 scraping with change detection (report-only)

4. **ClinicalTrials.gov Integration**
   - **Status:** Manual weekly checks
   - **Impact:** Delayed detection of trial status changes
   - **Recommendation:** Automate weekly trial status scraping (requires NCT number identification)

5. **Cashflow Runway Projections**
   - **Status:** Manual quarterly 10-Q review
   - **Impact:** Delayed runway recalculation on new filings
   - **Recommendation:** Automate 10-Q extraction and runway recalculation

---

## Confirmation: Report-Only / No-Alert Behavior

### Safety Verification

✅ **This MAIA monitoring pack is REPORT-ONLY and does NOT:**
- Generate Ross alerts
- Send Telegram messages
- Send email
- Modify Windows scheduled tasks
- Trigger Windows scheduled tasks
- Connect to production alert infrastructure
- Provide investment advice (buy/sell/hold)

✅ **This monitoring pack ONLY:**
- Tracks changes vs. CP23D baseline
- Identifies PM review triggers
- Provides weekly manual checklist framework
- Suggests future automation opportunities

### Source Boundary Compliance

✅ **Roger's uploaded MAIA spreadsheet:** NOT USED
✅ **OpenInsider data:** NOT USED
✅ **Secrets protected:** .env file NOT read, NOT printed
✅ **Scheduled tasks:** NOT modified, NOT triggered

### Investment Recommendation Compliance

✅ **No buy/sell/hold recommendation:** This is research monitoring only
✅ **No price targets:** Valuation not in scope
✅ **No expected returns:** Not in scope

---

## Next Actions

### Immediate (Weekly Monitoring)

1. **Execute weekly monitoring checklist** (see [MAIA_weekly_monitoring_checklist.md](MAIA_weekly_monitoring_checklist.md))
2. **Check for new Form 4 filings** (CIK 0001878313)
3. **Check for new Form 144 filings** (CIK 0001878313)
4. **Check for new 13D/13G filings** (CIK 0001878313)
5. **Review MAIA IR press releases** for clinical/regulatory updates
6. **Manual price check** vs. $1.50 offering price (if time permits)

### As-Filed (Event-Triggered Monitoring)

1. **Monitor SEC EDGAR RSS** for new Form 8-K material events
2. **Monitor MAIA IR** for press releases
3. **Check ClinicalTrials.gov** for trial status changes (weekly)

### Quarterly (Financial Monitoring)

1. **Review new 10-Q/10-K filings** for cash, burn rate, working capital
2. **Recalculate cash runway** vs. baseline (19.4 months from 2026-03-31)
3. **Check for going-concern language** changes
4. **Review any new financing filings** (S-3, 424B5, private placement)

### Engineering (Infrastructure Improvements)

1. **Integrate 13F InfoTable XML matching** (high priority)
2. **Implement market price/volume tracking** (high priority)
3. **Automate Form 4 daily scraping** (medium priority)
4. **Integrate ClinicalTrials.gov monitoring** (medium priority)
5. **Automate cashflow runway projections** (medium priority)

---

## Summary

**Current Status:** ✅ Baseline established, monitoring framework active, report-only behavior verified

**Top Priority:** THIO-104 Phase 3 data timing disclosure (critical unknown)

**Key Strengths:**
- Perfect insider buying pattern (100/100 score, 134 purchases, 0 sales)
- Strong cash position ($34.41M) with adequate near-term runway (19.4 months)
- FDA Fast Track for lead asset (THIO-104)
- Recent insider purchase (2026-06-01, 8 days ago)

**Key Risks:**
- THIO-104 data timing not disclosed (cannot assess financing risk)
- ~43% dilution overhang from options/warrants
- Limited institutional visibility (13F gap, beneficial ownership blockers)
- Potential additional raise if data timing exceeds runway

**Monitoring Posture:** Strong insider-evidence / high clinical-timing uncertainty profile

**Next Checkpoint Recommendation:** CP23F 13F InfoTable XML matching integration (high priority)

---

**End of Baseline Status Report**

**Checkpoint:** CP23E
**Generated:** 2026-06-10
**Based on:** CP23D MAIA Full Synthesis Packet
**Status:** Report-only monitoring (no alerts)
